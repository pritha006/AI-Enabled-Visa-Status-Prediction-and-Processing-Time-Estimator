...
print("Processing time model saved!")
import pandas as pd
import joblib
from sklearn.model_selection import train_test_split
...
education = st.sidebar.selectbox("Education Level", ["High School", "Bachelor's", "Master's"])
experience = st.sidebar.radio("Has Job Experience?", ["Yes", "No"])
full_time = st.sidebar.radio("Full-Time Position?", ["Yes", "No"])

st.sidebar.header("🏢 Employer Information")
employees = st.sidebar.number_input("Number of Employees", min_value=1)
...
with col1:
    ...
    if st.button("🔮 Predict Processing Time"):
        if os.path.exists("processing_time_model.pkl"):
            model = joblib.load("processing_time_model.pkl")

            # Dummy prediction (replace later)
            predicted_days = int(annual_wage / 1000 + company_age * 2)

            st.markdown(f"<p class='result'>Estimated Processing Time: {predicted_days} days</p>", unsafe_allow_html=True)
